...
<skill>
<name>floor-plan-backend</name>
<description>Develop the backend that converts PNG floor-plan images to 3D-ready FloorPlan JSON using LLM (GLM-4.7). Use when building POST /api/floor-plan/from-image, GET /api/floor-plan, or the Skill pipeline (Skill-A/C/D/E) that produces FloorPlan JSON compatible with floorPlan.ts and Indoor3D. Includes GLM-4.7 config (baseUrl, model; apiKey from env) and schema alignment with the repo README.</description>
<location>project</location>
</skill>
