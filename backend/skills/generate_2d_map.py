"""
建筑图纸墙体识别：颜色 + 线宽区分墙体与标注。
可先由 LLM 分析图片输出墙体识别参数（格式同 wall_detection_config），未输出时使用默认配置。
"""
...
from backend.services.llm_client import call_glm_vision_wall_config

_cfg = WALL_DETECTION_CONFIG
ELEMENT_WALL = "wall"


def _resolve_config(img_bgr: np.ndarray) -> dict[str, Any]:
    """
    优先用 LLM 分析图片得到墙体识别配置，与默认配置合并；LLM 无输出则用默认。
    """
    default = copy.deepcopy(WALL_DETECTION_CONFIG)
    try:
        _, buf = cv2.imencode(".png", img_bgr)
        b64 = base64.b64encode(buf.tobytes()).decode("ascii")
        llm_config = call_glm_vision_wall_config(b64)
        if llm_config:
            overridden = list(llm_config.keys())
            for k, v in llm_config.items():
                default[k] = v
            print(f"[FloorPlanElements] 使用 LLM 分析配置，共 {len(llm_config)} 项覆盖默认值: {overridden}")
            return default
        print("[FloorPlanElements] 未获取到 LLM 配置，使用默认 WALL_DETECTION_CONFIG")
    except Exception as e:
        print(f"[FloorPlanElements] LLM 配置解析跳过: {e}")
    return default


def _is_grey_color(bgr: np.ndarray, config: dict[str, Any] | None = None) -> np.ndarray:
    """灰/黑：BGR 三通道接近且均值在范围内。"""
    ...
    is_grey = (
        (channel_diff < cfg["grey_channel_diff_max"])
        & (channel_mean >= cfg["grey_wall_lo"])
        & (channel_mean <= cfg["grey_wall_hi"])
    )
    return is_grey


def _is_red_color(bgr: np.ndarray, config: dict[str, Any] | None = None) -> np.ndarray:
    """红色：R 明显高于 G、B（排除楼梯等）。"""
    cfg = config if config is not None else _cfg
    b, g, r = bgr[:, :, 0], bgr[:, :, 1], bgr[:, :, 2]
    return (
...
def _binary_grey_walls(img_bgr: np.ndarray, config: dict[str, Any] | None = None) -> np.ndarray:
    """只保留灰/黑像素（墙体线），排除白背景与红色。"""
    is_grey = _is_grey_color(img_bgr, config)
    is_red = _is_red_color(img_bgr, config)
...
def _is_wall_contour(contour: np.ndarray, total_px: int, config: dict[str, Any] | None = None) -> bool:
    """
    ...
    if area < 1e-6:
        return False
    min_wall_area = max(
        cfg["min_wall_area_abs"],
        int(total_px * cfg["min_wall_area_ratio"]),
    )
    ...
    return (
        thin_ratio <= cfg["wall_max_thin_ratio"]
        or thickness >= cfg["wall_min_thickness_px"]
    )
...
def _find_contours_from_binary(
    ...
    cfg = config if config is not None else _cfg
    so = cfg["morph_open_size"]
    sc_min, sc_max = cfg["morph_close_size_min"], cfg["morph_close_size_max"]
...
def classify_floor_plan_elements(
    ...
    config = _resolve_config(img)
    h, w = img.shape[:2]
    total_px = w * h
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    k_size = max(3, min(9, min(w, h) // config["kernel_size_divisor"]))

    walls: list[list[list[float]]] = []

    img_grey = _binary_grey_walls(img, config)
    grey_count = int(np.sum(img_grey == 255))
    if grey_count < total_px * config["grey_white_ratio_use_fallback"]:
        b, g, r = img[:, :, 0], img[:, :, 1], img[:, :, 2]
        is_red = _is_red_color(img, config)
        ch_max = np.maximum(np.maximum(b, g), r)
        ch_min = np.minimum(np.minimum(b, g), r)
        is_low_sat = (ch_max - ch_min) < config["low_sat_channel_diff"]
        img_grey = np.where(
            (gray >= config["grey_wall_lo"])
            & (gray <= config["grey_wall_hi"])
            & ~is_red
            & is_low_sat,
            255, 0,
        ).astype(np.uint8)
    if np.sum(img_grey == 255) < total_px * config["grey_white_ratio_use_otsu"]:
        _, otsu = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        if np.mean(otsu) > 127:
            otsu = 255 - otsu
        is_red = _is_red_color(img, config)
        is_grey = _is_grey_color(img, config)
    ...
    contours, _ = _find_contours_from_binary(img_grey, k_size, total_px=total_px, config=config)
    for cnt in contours:
        if not _is_wall_contour(cnt, total_px, config):
            continue
    ...
    if not walls and contours:
        min_area_loose = max(
            config["fallback_min_area_abs"],
            int(total_px * config["fallback_min_area_ratio"]),
        )
        for cnt in contours:
            ...
            if thin <= config["fallback_max_thin_ratio"] or thickness >= config["fallback_min_thickness_px"]:
                poly = _contour_to_normalized_polygon(cnt, w, h)
                if len(poly) >= 2:
                    walls.append(poly)
...
def _get_wall_contours_from_img(
    ...
    img_binary = _binary_grey_walls(img, cfg)
    ratio_sec = cfg["grey_ratio_secondary"]
    if np.sum(img_binary == 255) < (w * h * ratio_sec):
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        img_binary = np.where(
            (gray >= cfg["grey_wall_lo"]) & (gray <= cfg["grey_wall_hi"]),
            255, 0,
        ).astype(np.uint8)
        is_red = _is_red_color(img, cfg)
        ...
        if np.sum(img_binary == 255) < (w * h * ratio_sec):
            ...
            is_grey = _is_grey_color(img, cfg)
    ...
    kernel_size = max(3, min(9, min(w, h) // cfg["kernel_size_divisor"]))
    ...
    total_px = w * h
    wall_contours = [c for c in contours if _is_wall_contour(c, total_px, cfg)]
...
def get_wall_contours(
    ...
    config = _resolve_config(img)
    return _get_wall_contours_from_img(img, annotation_boxes_px, config)
...
def wall_edge_detection(
    ...
    """
    建筑图纸墙体描边：先由 LLM 分析图片得到识别参数（可选），再描墙体轮廓。
    与 outline-walls 一致，使用同一套轮廓管线后描边。
    """
    img = _load_image_for_walls(image_input)
    config = _resolve_config(img)
    wall_contours, w, h = _get_wall_contours_from_img(img, annotation_boxes_px, config)
